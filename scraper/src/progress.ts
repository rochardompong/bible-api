import type { ScrapeProgress, ScrapePhase } from "@bible-mirror/types";
...
const DEFAULT_PROGRESS: ScrapeProgress = {
  ...
  current_phase: "init",
  phase_progress: { bible_index: 0, book_index: 0, chapter_index: 0, verse_index: 0 },
  selected_bible_ids: [],
  completed_phases: [],
...
export async function loadProgress(): Promise<ScrapeProgress> {
  _progress = (await r2Get<ScrapeProgress>(PROGRESS_KEY)) ?? { ...DEFAULT_PROGRESS };
  console.log(`[Progress] Phase: ${_progress.current_phase} | Total: ${_progress.total_requests_made} req`);
  return _progress;
}

export async function saveProgress(updates: Partial<ScrapeProgress>): Promise<void> {
  if (!_progress) throw new Error("Progress not loaded");
  _progress = { ..._progress, ...updates, last_run: new Date().toISOString() };
